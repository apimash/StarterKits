...
namespace TwitterAPIWinPhone8Lib.TwitterOAuth
{
    ...
    public class TweetOAuth
    {
        ...
        private object _authWebPage, _authWebControl;
        ...
        public async Task AuthenticateTwitterByOAuth(object authWebControl = null, object authWebPage = null)
        {
            _authWebControl = authWebControl;
            _authWebPage = authWebPage;
        ...
        public string OAuthSignature(string SigBaseString, SignatureAuthType signType)
        {
            ...
            byte[] keyBuffer = Encoding.UTF8.GetBytes(signingKey);
            HMACSHA1 hmacSha1Provider = new HMACSHA1();
            hmacSha1Provider.Key = keyBuffer;
            byte[] DataToBeSigned = Encoding.UTF8.GetBytes(SigBaseString);
            //HMACSHA1 hmacSha1Provider = new HMACSHA1(keyBuffer);
            byte[] SignatureBuffer = hmacSha1Provider.ComputeHash(DataToBeSigned);
        ...
        public async Task<string> OAuthRequestToken(string requestUrl)
        {
            ...
            {
                // HttpResponseMessage webHttpResponse = await webClient.GetAsync(requestUrl);
        ...
        public async Task RetrieveWebOAuthTokens(string authorizeResponse)
        {
        ...
        //public void AuthWebCallBack(System.Object sender, System.Windows.Navigation.NavigationEventArgs e)
        //{
        //    Microsoft.Phone.Controls.WebBrowser _webBrowser = (Microsoft.Phone.Controls.WebBrowser)sender;
        //    Microsoft.Phone.Controls.PhoneApplicationPage _webPage = (Microsoft.Phone.Controls.PhoneApplicationPage)_authWebPage;
        ...
        //        RetrieveWebOAuthTokens(tokenResult);
        //        RetrieveWebOAuthTokens(e.Uri.ToString());

        //        System.Windows.Deployment.Current.Dispatcher.BeginInvoke(() =>
        //        {
        //            _webPage.NavigationService.Navigate(new Uri(_webBrowser.Base
        //        });
        //    }
        //}
        ...
        public void StoreOAuthUserInfo()
        {
            ...
            //ApplicationData.Current.LocalSettings.DeleteContainer("TwitterAPIMash");
                ...
                if (!appData.Contains(CommonProperties.TwitterAPI_DataStorage)) appData.Add(CommonProperties.TwitterAPI_DataStorage, new Dictionary<string, string>());
                
                Dictionary<string, string> appLocalData = appData[CommonProperties.TwitterAPI_DataStorage] as Dictionary<string, string>;
                
                if (appLocalData.ContainsKey("OAuthConsumerKey")) { appLocalData["OAuthConsumerKey"] = OAuth.OAuthTypes.OAuthConsumerKey; }
                else { appLocalData.Add("OAuthConsumerKey", OAuth.OAuthTypes.OAuthConsumerKey); }

                if (appLocalData.ContainsKey("OAuthConsumerSecret")) { appLocalData["OAuthConsumerSecret"] = OAuth.OAuthTypes.OAuthConsumerSecret; }
                else { appLocalData.Add("OAuthConsumerSecret", OAuth.OAuthTypes.OAuthConsumerSecret); }

                if (appLocalData.ContainsKey("OAuthAccessToken")) { appLocalData["OAuthAccessToken"] = OAuth.OAuthTypes.OAuthAccessTokenKey; }
                else { appLocalData.Add("OAuthAccessToken", OAuth.OAuthTypes.OAuthAccessTokenKey); }

                if (appLocalData.ContainsKey("OAuthAccessSecret")) { appLocalData["OAuthAccessSecret"] = OAuth.OAuthTypes.OAuthAccessTokenSecretKey; }
                else { appLocalData.Add("OAuthAccessSecret", OAuth.OAuthTypes.OAuthAccessTokenSecretKey); }

                if (appLocalData.ContainsKey("OAuthVerifier")) { appLocalData["OAuthVerifier"] = OAuth.OAuthTypes.OAuthVerifierKey; }
                else { appLocalData.Add("OAuthVerifier", OAuth.OAuthTypes.OAuthVerifierKey); }

                if (appLocalData.ContainsKey("UserID")) { appLocalData["UserID"] = TweetAuthConstants.OAuthUserID; }
                else { appLocalData.Add("UserID", TweetAuthConstants.OAuthUserID); }

                if (appLocalData.ContainsKey("UserScreenName")) { appLocalData["UserScreenName"] = TweetAuthConstants.OAuthUserScreenName; }
                else { appLocalData.Add("UserScreenName", TweetAuthConstants.OAuthUserScreenName); }
