...
Public Class LayoutAwarePage
    ...
    Public Shared ReadOnly DefaultViewModelProperty As DependencyProperty =
        DependencyProperty.Register("DefaultViewModel",
                                    GetType(IObservableMap(Of [String], [Object])),
...
#Region "Navigation support"
    ...
    Private Sub CoreDispatcher_AcceleratorKeyActivated(sender As CoreDispatcher, args As AcceleratorKeyEventArgs)
        Dim virtualKey__1 = args.VirtualKey

        ' Only investigate further when Left, Right, or the dedicated Previous or Next keys
        ' are pressed
        If (args.EventType = CoreAcceleratorKeyEventType.SystemKeyDown OrElse args.EventType = CoreAcceleratorKeyEventType.KeyDown) AndAlso (virtualKey__1 = VirtualKey.Left OrElse virtualKey__1 = VirtualKey.Right OrElse CInt(virtualKey__1) = 166 OrElse CInt(virtualKey__1) = 167) Then
            ...
            If (CInt(virtualKey__1) = 166 AndAlso noModifiers) OrElse (virtualKey__1 = VirtualKey.Left AndAlso onlyAlt) Then
                ' When the previous key or Alt+Left are pressed navigate back
                args.Handled = True
                Me.GoBack(Me, New RoutedEventArgs())
            ElseIf (CInt(virtualKey__1) = 167 AndAlso noModifiers) OrElse (virtualKey__1 = VirtualKey.Right AndAlso onlyAlt) Then
...
#Region "Visual state switching"
    ...
    Private Sub WindowSizeChanged(sender As Object, e As WindowSizeChangedEventArgs)
    ...
    Public Sub InvalidateVisualState()
        If Me._layoutAwareControls IsNot Nothing Then
            ...
            For Each layoutAwareControl As Object In Me._layoutAwareControls
...
#End Region
    ...
    Private Class ObservableDictionary(Of K, V)
        ...
        Private Class ObservableDictionaryChangedEventArgs
            ...
            Private m_CollectionChange As CollectionChange
            Private m_Key As K
        ...
        Private Sub InvokeMapChanged(change As CollectionChange, key As K)
            '' in c# was a check if mapchanged was null
        ...
        Public Sub Clear() Implements ICollection(Of KeyValuePair(Of K, V)).Clear
            ...
            For Each key As Object In priorKeys
        ...
        Public Sub CopyTo(array As KeyValuePair(Of K, V)(), arrayIndex As Integer) Implements ICollection(Of KeyValuePair(Of K, V)).CopyTo
            ...
            For Each pair As Object In Me._dictionary
        ...
        Public Function Remove(item As KeyValuePair(Of K, V)) As Boolean Implements ICollection(Of KeyValuePair(Of K, V)).Remove
            ...
            Return False
        End Function
        Public Function Remove1(key As K) As Boolean Implements IDictionary(Of K, V).Remove
